...
//		response.getWriter().append("Served at: ").append(request.getContextPath());
		 ...
		 *   - 사용자가 입력한 값, 요청시 전송방식, ip 주소, 기타...
		 * 
		 * 2. HttpServletResponse
		 *   - 응답정보를 담아줄 객체.
		 *   - 응답정보로 동적 html 페이지를 생성하여 반환.
		 * 
		 * */
		// 1. 사용자가 입력한 값 가져오기.
		...
		// 2. 응답 메세지 작성
		// html
		response.setContentType("text/html; charset=utf-8;"); // 응답 데이터의 형식 (필수)
		PrintWriter out = response.getWriter(); // outputStream(Writer)
...
//		doGet(request, response);
		/*
		 * POST 방식의 요청시 데이터는 http body 영역에 "인코딩"되어 넘어온다. 기본 인코딩 설정은 ISO-8859-1로, utf-8이 아님.
